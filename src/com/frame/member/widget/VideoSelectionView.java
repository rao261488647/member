...
import com.frame.member.widget.HorizontalScrollViewEx.OnFlingListener;
...
public class VideoSelectionView extends RelativeLayout implements OnFlingListener, android.widget.CompoundButton.OnCheckedChangeListener {
	...
	private void init() {
		...
		View view = LayoutInflater.from(getContext()).inflate(R.layout.view_video_selection, this);
		...
		mRadioGroupBackground = (RadioGroup) view.findViewById(R.id.radiogroup_background);
...
//						CrashUncaughtException.sendFfmpegLog();
				...
				cmd = String.format("ffmpeg -d stdout -loglevel verbose -i \"%s\" -r 1/%.1f -start_number %d -vframes %d -s %dx%d %s \"%s", tempVideo, r, startIndex, vframes, mThumbWidth, mThumbWidth, ffmpeg, mThubmDir.getPath()) + File.separator + "thumb_%d.jpg\"";
