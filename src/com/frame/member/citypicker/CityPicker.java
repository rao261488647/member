package com.frame.member.citypicker;
...
import com.frame.member.citypicker.ScrollerNumberPicker.OnSelectListener;
...
public class CityPicker extends LinearLayout {
	...
	private String couny_name = "北京";
	private String city_code = "101010100";
	...
	private void getaddressinfo() {
		...
		// 读取文件里的城市信息
		String area_str = CityPickerFileUtil.readAssets(context, "area.json");
		// 解析城市信息
	...
	protected void onFinishInflate() {
		...
		cityPicker.setData(city_map.get("北京"));
		counyPicker.setData(parser.toArrayList(couny_map.get("北京市")));
		...
		provincePicker.setOnSelectListener(new OnSelectListener() {
			...
			public void endSelect(int id, String text) {
				...
				if (tempProvinceIndex != id) {
					...
					// 设置县的数据内容
					counyPicker.setData(parser.toArrayList(couny_map
							.get(cityPicker.getSelectedText())));
					counyPicker.setDefault(0);
					couny_name = counyPicker.getSelectedText();
					city_code = couny_map.get(city_name).get(0).getId();
		...
		cityPicker.setOnSelectListener(new OnSelectListener() {
			...
			public void endSelect(int id, String text) {
				...
				if (temCityIndex != id) {
					...
					city_name = text;
					// 设置县的数据内容
					counyPicker.setData(parser.toArrayList(couny_map.get(text)));
					counyPicker.setDefault(0);
		...
		counyPicker.setOnSelectListener(new OnSelectListener() {
			...
			public void endSelect(int id, String text) {
				...
				if (tempCounyIndex != id) {
					...
					couny_name = text;
					// 在县集合中得到城市的天气代码
					city_code = couny_map.get(city_name).get(id).getId();
			...
			case REFRESH_VIEW:
				if (onSelectingListener != null)
					onSelectingListener.selected(true, province_name,
							city_name, couny_name, city_code);
				break;
			default:
				break;
			}
